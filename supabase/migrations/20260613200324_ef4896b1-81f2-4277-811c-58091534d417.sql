...
CREATE UNIQUE INDEX IF NOT EXISTS idx_socios_user_id
  ON public.socios(user_id)
  WHERE user_id IS NOT NULL;

DROP POLICY IF EXISTS "Franqueado vê o próprio registro de sócio" ON public.socios;
...
DROP POLICY IF EXISTS "Franqueado vê documentos da própria unidade" ON public.unidade_documentos;
...
DROP POLICY IF EXISTS "Franqueado vê plantas da própria unidade" ON storage.objects;
...
GRANT ALL ON public.obra_checklist_itens TO service_role;
...
GRANT ALL ON public.financeiro_mensal TO service_role;
