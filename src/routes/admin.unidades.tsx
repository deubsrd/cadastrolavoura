...
function AdminUnidades() {
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    if (error)
      return toast.error(
        isTransientDbError(error) ? "Conexão instável. Tente novamente." : error.message,
      );
    setRows((prev) =>
      prev
        .map((r) => (r.id === editing.id ? (data as Unidade) : r))
  ...
  const load = async () => {
    setLoading(true);
    const { data, error } = await withDbRetry(() =>
      supabase.from("unidades").select("*").order("numero"),
    );
    setLoading(false);
    ...
    try {
      const { data, error } = await withDbRetry(
        () =>
          supabase
            .from("unidades")
            .insert({ numero: numero.trim(), nome: nome.trim() || null })
            .select("*")
            .single(),
        2,
      );
      if (error)
        return toast.error(
          isTransientDbError(error)
            ? "A conexão demorou demais. Tente novamente em alguns segundos."
            : error.message,
        );
      setNumero("");
      setNome("");
      ...
      setRows((prev) =>
        [...prev, data as Unidade].sort((a, b) => a.numero.localeCompare(b.numero)),
      );
    } catch {
  ...
  const toggle = async (u: Unidade) => {
    const { error } = await withDbRetry(() =>
      supabase.from("unidades").update({ ativo: !u.ativo }).eq("id", u.id),
    );
    if (error)
      return toast.error(
        isTransientDbError(error)
          ? "Conexão com o banco instável. Tente novamente em alguns segundos."
          : error.message,
      );
    load();
  };
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Gerencie as unidades disponíveis para cadastro.
        </p>
      </div>
                  ...
                  <TableBody>
                    {socios.map((s) => (
                      <TableRow key={s.id}>
                        ...
                        <TableCell>
                          <Badge variant={s.tipo === "administrador" ? "default" : "secondary"}>
                            {s.tipo}
                          </Badge>
                        </TableCell>
